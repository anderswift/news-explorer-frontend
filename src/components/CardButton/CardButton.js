...
function CardButton({ icon }) {
  ...
  return (
      <button type="button" aria-label="Save Article" className="button card__button" onClick={handleSave}>
        <svg className={`button__icon card__button-icon ${isSaved ? ' card__button-icon_saved' : ''}`} width="24" height="24" viewBox="0 0 24 24">
          {icon === 'save' && 
